//! Intermediate Representation (IR) for panlabel.
//!
//! This module defines the canonical, format-agnostic representation of
//! object detection datasets. It serves as the central "hub" that all
//! format conversions pass through, similar to how Pandoc uses an internal
//! AST for document conversion.
//!
//! # Design Principles
//!
//! 1. **Type Safety**: Use newtypes and marker types to prevent common errors
//!    at compile time (e.g., mixing pixel and normalized coordinates).
//!
//! 2. **Canonical Format**: The IR uses a single, well-defined coordinate
//!    system (XYXY in pixel space) to avoid ambiguity.
//!
//! 3. **Permissive Construction**: IR types allow "invalid" data to be
//!    represented (e.g., negative coordinates), so that validation can
//!    report issues rather than panic during parsing.
//!
//! # Example
//!
//! ```
//! use panlabel::ir::{Dataset, Image, Category, Annotation, BBoxXYXY, Pixel};
//!
//! let dataset = Dataset {
//!     images: vec![Image::new(1u64, "image.jpg", 640, 480)],
//!     categories: vec![Category::new(1u64, "person")],
//!     annotations: vec![
//!         Annotation::new(
//!             1u64, 1u64, 1u64,
//!             BBoxXYXY::<Pixel>::from_xyxy(10.0, 20.0, 100.0, 200.0),
//!         )
//!     ],
//!     ..Default::default()
//! };
//! ```

mod bbox;
mod coord;
mod ids;
mod io_adapter_common;
pub mod io_automl_vision_csv;
mod io_bbox_adapters_common;
pub mod io_bdd100k_json;
pub mod io_cityscapes_json;
pub mod io_cloud_annotations_json;
pub mod io_coco_json;
pub mod io_createml_json;
pub mod io_cvat_xml;
pub mod io_datumaro_json;
pub mod io_edge_impulse_labels;
pub mod io_hf_imagefolder;
#[cfg(feature = "hf-parquet")]
pub mod io_hf_parquet;
pub mod io_json;
pub mod io_kaggle_wheat_csv;
pub mod io_kitti;
pub mod io_label_studio_json;
pub mod io_labelbox_json;
pub mod io_labelme_json;
pub mod io_marmot_xml;
pub mod io_oidv4_txt;
pub mod io_openimages_csv;
pub mod io_openlabel_json;
pub mod io_retinanet_csv;
pub mod io_sagemaker_manifest;
pub mod io_scale_ai_json;
mod io_super_json_common;
pub mod io_superannotate_json;
pub mod io_supervisely_json;
pub mod io_tfod_csv;
pub mod io_tfrecord;
pub mod io_udacity_csv;
pub mod io_unity_perception_json;
pub mod io_v7_darwin_json;
pub mod io_via_csv;
pub mod io_via_json;
pub mod io_voc_xml;
pub mod io_vott_csv;
pub mod io_vott_json;
pub mod io_wider_face_txt;
pub mod io_yolo;
pub mod io_yolo_keras_txt;
mod model;
mod space;

// Re-export core types for convenient access
pub use bbox::BBoxXYXY;
pub use coord::Coord;
pub use ids::{AnnotationId, CategoryId, ImageId, LicenseId};
pub use model::{Annotation, Category, Dataset, DatasetInfo, Image, License};
pub use space::{Normalized, Pixel};
